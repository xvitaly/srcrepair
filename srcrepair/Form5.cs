...
namespace srcrepair
{
    public partial class frmInstaller : Form
    {
        ...
        {
            // Устанавливаем файл...
            File.Copy(FileName, CoreLib.IncludeTrDelim(GV.FullGamePath + SubDir) + Path.GetFileName(FileName), true);
        }
        ...
        private void InstallSprayNow(string FileName)
        {
            ...
            string CDir = CoreLib.IncludeTrDelim(Path.GetDirectoryName(FileName)); // Получаем каталог с файлами для копирования...
            string FPath = GV.FullGamePath + "materials" + Path.DirectorySeparatorChar + "vgui" + Path.DirectorySeparatorChar + "logos" + Path.DirectorySeparatorChar; // Получаем путь к каталогу назначения...
